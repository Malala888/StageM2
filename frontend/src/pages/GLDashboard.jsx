...
// ─── Cache mémoire ───
let glDashboardCache = null;
let glDashboardCacheTime = 0;
const CACHE_TTL_MS = 15000;

async function fetchGLDashboardData() {
  const now = Date.now();
  if (glDashboardCache && now - glDashboardCacheTime < CACHE_TTL_MS) {
    return glDashboardCache;
  }
  ...
  // Créer un objet utilisateur enrichi
  const user = {
    ...userData,
    brigade: brigade, // objet brigade complet
    section: section, // objet section complet
  };
  ...
  glDashboardCache = result;
  glDashboardCacheTime = now;
  return result;
}

// ─── Loader ───
export async function glDashboardLoader() {
  return fetchGLDashboardData();
}

// ─── ErrorElement ───
export function GLDashboardError() {
  const error = useRouteError();
  console.error('Erreur lors du chargement du tableau de bord GL:', error);
  return (
...
const GLDashboard = () => {
  ...
  // Nom de la brigade et de la section (maintenant des objets)
  ...
  const handleDemanderEmprunt = (materielId, nom) => {
    ...
    // Ici, on pourrait ouvrir un modal pour choisir la quantité et la durée
  };
          ...
          <main className="main">
            <div className="page-header">
              <div>
                ...
                <div className="sub">
                  Bienvenue, {user?.prenom || 'Garde Ligne'} {user?.nom || ''} <span className="role-badge">GL</span>
                </div>
              </div>
