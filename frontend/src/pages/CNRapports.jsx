...
// ─── Cache mémoire ───
let cnRapportsCache = null;
let cnRapportsCacheTime = 0;
const CACHE_TTL_MS = 15000;

async function fetchCNRapportsData() {
  const now = Date.now();
  if (cnRapportsCache && now - cnRapportsCacheTime < CACHE_TTL_MS) {
    return cnRapportsCache;
  }
  ...
  cnRapportsCache = result;
  cnRapportsCacheTime = now;
  return result;
}

// ─── Loader ───
export async function cnRapportsLoader() {
  return fetchCNRapportsData();
}

// ─── ErrorElement ───
export function CNRapportsError() {
  const error = useRouteError();
  console.error('Erreur lors du chargement des rapports CN:', error);
  return (
          ...
          <main className="main">
            <div className="page-header">
              <div>
                <h1>Mes Rapports</h1>
                <div className="sub">
                  Statistiques personnelles — <span className="role-badge">CN</span>
                </div>
              </div>
